...
- IoU 带 [0.1, 0.5] 过滤（低 IoU/纯背景与高 IoU/重复检测均排除）。
...
_QUERY_BOXES = torch.tensor(
    [
        [0.1, 0.1, 0.05, 0.05],  # q0: IoU = 0.0 < 0.1（纯背景）
        [0.5, 0.5, 0.1, 0.1],  # q1: IoU = 0.25 ∈ [0.1, 0.5]（难例带内）
        [0.5, 0.5, 0.2, 0.2],  # q2: IoU = 1.0 > 0.5（真实目标的重复检测）
...
class TestHardNegSelection:
    ...
    def test_iou_band_filtering(self) -> None:
        """带内（IoU 0.25）候选入选，纯背景（0）与重复检测（1.0）排除。"""
        hs = _hs()
        # q1 最高分且带内 → 入选；q0/q2 分数更高也必须被 IoU 带挡住
        scores = [5.0, 4.0, 6.0] + [0.0] * 7
        logits = _logits(scores)
        # 先验证测试数据自身的 IoU 假设
        iou, _ = box_iou(box_cxcywh_to_xyxy(_QUERY_BOXES), box_cxcywh_to_xyxy(_GT_BOX))
        assert iou[0, 0].item() == 0.0
        assert 0.1 <= iou[1, 0].item() <= 0.5
    ...
    def test_topk_order_ignores_background_and_stable(self) -> None:
        ...
        hs = _hs()
        # 构造两个带内候选：q1 前景分 4.0，q3 前景分 5.0（q3 需要进带内：
        # 给 q3 一个与 GT 部分重叠的框）
        boxes = _QUERY_BOXES.clone()
        boxes[3] = torch.tensor([0.5, 0.5, 0.15, 0.15])  # q3: IoU = 0.5625?
        iou, _ = box_iou(box_cxcywh_to_xyxy(boxes[3:4]), box_cxcywh_to_xyxy(_GT_BOX))
        # 若超出 0.5 上界则调整尺寸，保证测试数据自身落入带内
        if iou.item() > IOU_BAND_HIGH:
            boxes[3] = torch.tensor([0.5, 0.5, 0.13, 0.13])
            iou, _ = box_iou(box_cxcywh_to_xyxy(boxes[3:4]), box_cxcywh_to_xyxy(_GT_BOX))
    ...
    def test_random_features_exclude_matched_and_deterministic(self) -> None:
        ...
        assert hn.shape[0] == 0  # q1 被 matched 排除后无带内候选

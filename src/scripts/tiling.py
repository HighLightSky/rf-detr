...
class _TileDataset(Dataset):
    ...
    def __init__(
        ...
        Args:
            ...
            overlap: 滑窗重叠像素数。
        """
        self._items: list[tuple[str, Path, int, int, int, int]] = []
        ...
        for image_path in image_paths:
            ...
            for x0, y0 in tile_origins((width, height), resolution, overlap):
                ...
                self._items.append((image_path.stem, image_path, x0, y0, tile_w, tile_h))
    ...
    def __getitem__(self, index: int) -> tuple[str, int, int, int, int, torch.Tensor]:
        """解码并切片第 *index* 个 tile。

        Returns:
            ``(stem, x0, y0, tile_w, tile_h, tile_tensor)``：tile_tensor 为
            ``(C, tile_h, tile_w)`` uint8（零拷贝的连续视图）。
        """
        stem, image_path, x0, y0, tile_w, tile_h = self._items[index]
        ...
        return stem, x0, y0, tile_w, tile_h, torch.from_numpy(tile).permute(2, 0, 1)


def _tile_collate(
    batch: list[tuple[str, int, int, int, int, torch.Tensor]],
) -> tuple[list[str], torch.Tensor, torch.Tensor, torch.Tensor]:
    """把 tile 样本批化为 ``(stems, origins, sizes, tensors)``。
    ...
    Returns:
        ...
        ``sizes`` 为 ``(N, 2)`` 内容尺寸 ``(tile_w, tile_h)``；``tensors`` 为
        ``(N, 3, R, R)`` uint8 张量。
    """
    ...
    tensors = torch.stack([item[5] for item in batch])
    return stems, origins, sizes, tensors
...
def tile_predict_records(
    ...
    *,
    ...
    """大图滑窗切分推理：worker 预取切块，tile 混合批量前向，按类别 NMS 合并。
    ...
    经置信度阈值过滤 → 偏移到全图坐标（GPU 上完成）→ 逐张按类别 NMS 去重。
    ...
    # 每张大图累计的 (全图坐标框, 类别, 分数) 张量列表，循环结束后逐张 NMS
    per_image: dict[str, list[tuple[torch.Tensor, torch.Tensor, torch.Tensor]]] = {}
    total_tiles = len(dataset)
    tiles_done = 0
    bench_start = time.perf_counter()

    with torch.inference_mode():
        for stems, origins, sizes, tensors in loader:
            ...
            for i, (stem, (boxes, labels, scores)) in enumerate(
                ...
                # 偏移到全图坐标（GPU 上完成，NMS 后再统一转 CPU）
                per_image.setdefault(stem, []).append((boxes + offsets[i], labels, scores))
    ...
    # 逐张合并所有 tile 的框 → 按类别 NMS 去重 → 转 BoxRecord

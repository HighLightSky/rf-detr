...
# ------------------------------------------------------------------------
"""阶段 0（DIOR 版）：离线构建 DIOR 多模态原型产物（视觉 + 文本）。

与 ``stage0_build_proto_guidance.py``（SHWX 版）逻辑一致，仅按 DIOR 适配：
- 特征空间来源：DIOR 基线 checkpoint（0726-DIOR-rfdetr_medium，20 类 / 640 分辨率），
  保证视觉原型与训练骨干/projector 特征空间一致；
- 数据集：/home/liu/wzt/datasets/DIOR-rfdetr（COCO/Roboflow 格式）；
- 文本原型：CLIP 对 ``sscl/prompts/dior.yaml`` 提示词编码取平均（EOS/pooler 表征，
  与语义矩阵新方法一致）。
...
输出：
    ...
    ``meta`` 等键，由训练配置 proto_guidance_artifacts_path 引用。
"""
...
from rfdetr.sscl.prompts import DIOR_CLASS_NAMES, DIOR_CLASS_PROMPTS
...
# 与训练配方一致的 backbone checkpoint（DIOR 基线，特征空间与训练一致）
BASE_CHECKPOINT = str(Path("output/0804重标注之前的实验/0726-DIOR-rfdetr_medium/checkpoint_best_total.pth").resolve())

# DIOR 数据集（Roboflow COCO 布局；与 configs/experiments/proto_guidance_DIOR-allon.yaml 一致）
...
OUTPUT_FILE = str(Path("data/proto_guidance_dior.pt").resolve())
...
def main() -> None:
    ...
    for class_id in range(NUM_CLASSES):
        ...
        print(
            f"  类别 {class_id} ({DIOR_CLASS_NAMES.get(class_id + 1, '?')}): "
            f"{class_features.shape[0]} 个实例 -> {int(valid.sum())} 个槽位"
        )

    # 文本原型：CLIP 对遥感提示词编码取平均（多 prompt 平均，768 维不归一化）。
    # 模型路径由 encode_class_text_embeddings 自动解析（仓库内 data/clip/ 优先）。
    print("编码 CLIP 文本原型 ...")
    text_prototypes = encode_class_text_embeddings(DIOR_CLASS_PROMPTS, device=DEVICE)
    ...
    class_names = [DIOR_CLASS_NAMES.get(c, f"c{c}") for c in range(1, NUM_CLASSES + 1)]
    ...
    print(
        f"完成: {OUTPUT_FILE}（视觉原型 {tuple(visual_prototypes.shape)}，"

...
普通 supervised contrastive learning 对所有负样本一视同仁。SSCL 的改动是：对不同类别负样本，根据语义相似度矩阵 $\Phi$ 赋予不同权重。

对 foreground query feature $h_i$，先进行归一化：

$$
u_i = \frac{h_i}{\|h_i\|_2}
$$

对于 anchor $i$，正样本集合为同类别 query：
...
+
\sum_{j \in N(i)}
\exp(\omega_{i,j} u_i^\top u_j / \tau)
}
$$

其中 $\mathcal{A}$ 是有效 anchor 集合，$\tau$ 是对比学习温度，$\omega_{i,j}$ 来自类别语义相似度：

$$
\omega_{i,j} = 1 + \rho \cdot \Phi'_{y_i,y_j}
$$

这里 $\rho$ 控制语义先验对负样本强度的放大程度。若两个类别语义相似，则 $\Phi'_{y_i,y_j}$ 较大，负样本项的权重更高，模型会更强地把它们分开。

为了避免训练不稳定，建议将 $\omega_{i,j}$ 限制在一个保守范围内：

$$
\omega_{i,j} \in [1, \omega_{\max}]
$$

例如 $\omega_{\max}=1.5$ 或 $2.0$。
